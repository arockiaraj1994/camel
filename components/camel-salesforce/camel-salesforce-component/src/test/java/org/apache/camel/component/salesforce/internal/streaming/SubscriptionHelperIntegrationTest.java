...
public class SubscriptionHelperIntegrationTest {
    ...
    public SubscriptionHelperIntegrationTest() throws SalesforceException {
        ...
        System.out.println("Port for wireshark to filter: " + server.port());

        final String instanceUrl = "http://localhost:" + server.port();

        server.replyTo("POST", "/services/oauth2/token", "{\"instance_url\":\"" + instanceUrl + "\",\"access_token\":\"token\"}");
        ...
        salesforce.setHttpProxyHost("localhost");
        salesforce.setHttpProxyPort(8866);
    ...
    void shouldResubscribeOnConnectionFailures() throws InterruptedException {
        ...
        verify(consumer, Mockito.timeout(100)).processMessage(any(ClientSessionChannel.class), messageForAccountCreationWithName("Test Account"));
        ...
        verify(consumer, Mockito.timeout(2000)).processMessage(any(ClientSessionChannel.class), messageForAccountCreationWithName("New Test Account"));

        verifyNoMoreInteractions(consumer);
    }

    @Test
    void shouldResubscribeOnHelperRestart() throws InterruptedException {
        // handshake and connect
        subscription.start();
